...
const getSetting = async () => {
  let result = await proxy.Request({
    url: proxy.globalInfo.api.getSetting,
  });
  if (!result) {
    return;
  }
  //返回值是一个大的对象，其中的值也是对象，需用两层循环将每个值取出来给formData赋值
...
const saveSetting = async () => {
  formDataRef.value.validate(async (valid) => {
    ...
    let result = await proxy.Request({
      url: proxy.globalInfo.api.saveSetting,
        ...
        <el-col :span="12">
          <el-form-item label="每天可点赞数量" prop="likeDayCountThreshold">
            <el-input
              ...
              v-model.trim="formData.likeDayCountThreshold"
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
